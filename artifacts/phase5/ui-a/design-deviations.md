...
| S6 canvas color references | Token indirection only; no visual or behavioral change | S1's permanent namespace rule required the canvas' existing AppKit semantic colors (`controlAccentColor`, `systemRed`, white, and the existing 60% accent stem) to be referenced through equivalent `Theme` tokens. Selection-chrome geometry, drawing operations, gestures, snapping math, and tool behavior are unchanged. |
