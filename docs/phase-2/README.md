...
| Stack passes identified risk spikes | Conditional | Apple Silicon automated gates pass; Intel, pressure input, VoiceOver and physical-display checks require target hardware/manual execution |

Phase 3 may begin for core/document vertical slices. Release claims for Intel or
accessibility remain blocked until the external gates in `technical-spikes.md` are
recorded. Spikes are test fixtures and evidence, not production feature promises.
