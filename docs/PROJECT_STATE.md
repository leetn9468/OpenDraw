...
| BLOCK-006 | Hosted-CI execution and ratio proof | `OPEN` | CI #1 at `834526c` ran on `macos-15` but did not qualify: benchmark and startup-memory exited 127 at final ripgrep-based fixture assertions, and nightly-reliability was skipped on push. The portable POSIX-grep/workflow-dispatch correction requires a new manually dispatched run in which all eight jobs execute and pass, artifacts/run URL are retained, and a clearly hosted baseline/comparison is bootstrapped. This simultaneously closes hosted components of BLOCK-003/004/005. |
