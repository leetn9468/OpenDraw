...
| BLOCK-006 | Hosted-CI execution and ratio proof | `OPEN` | Closure requires every job defined by the workflow to execute and pass in one manually dispatched run on the final revision, with retained artifacts/run URL and a clearly hosted baseline/comparison bootstrap. Partial-green runs never qualify. Failed attempts retained: CI #1 (`29315797697`, exit-127 portability defect) and CI #4 (`29327190429`, release descriptor race plus compressible memory fixture). Corrections `b6cee43` and `ce3b4b3` have complete required local reruns; no qualifying run has been dispatched on the new revision. |
| BLOCK-007 | Fresh final-revision full verification battery | `CLOSED LOCALLY` | The complete debug/release/ASan/coverage and local R4 gate battery passed on current build-input revision `ce3b4b3`; artifact mapping is in `revision-map.md`. |
...
| BLOCK-011 | Exact revision accounting | `CLOSED` | `revision-map.md` maps the complete battery to `ce3b4b3` and retaining evidence commit `8da5ee8`; `revision-scope-proof.txt` confirms the three changed code/test paths, frozen-VERIFY non-modification, macOS 15 runner pinning, and ancestry from `f6ec81a`. |
