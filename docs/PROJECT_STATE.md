...
| BLOCK-006 | Hosted-CI execution and ratio proof | `OPEN` | CI #1 at `834526c` ran on `macos-15` but did not qualify: benchmark and startup-memory exited 127 at final ripgrep-based fixture assertions, and nightly-reliability was skipped on push. Portable fix `b6cee43` uses equivalent POSIX grep, enables dispatched reliability, updates Node.js 24 action majors, and passes every affected local rerun. A new manually dispatched run must execute all eight jobs green, retain artifacts/run URL, and bootstrap a clearly hosted baseline/comparison. This simultaneously closes hosted components of BLOCK-003/004/005. |
