...
| BLOCK-006 | Hosted-CI execution and ratio proof | `OPEN` | Closure requires every job defined by the workflow to execute and pass in one manually dispatched run on the final revision, with retained artifacts/run URL and a clearly hosted baseline/comparison bootstrap. Failed attempts retained: CI #1 (`29315797697`, portability), CI #4 (`29327190429`, descriptor/memory-fixture defects), and CI #5 (`29340697610`, hosted observations incorrectly compared with the owner-local baseline). Workflow-only fixes `b57293c`/`59eec99` implement the ratified hosted bootstrap/enforcement split and fail closed on incomplete candidates; no qualifying run has been dispatched on the final revision. |
| BLOCK-007 | Fresh final-revision full verification battery | `CLOSED LOCALLY` | The complete local battery passed at code revision `ce3b4b3`. Later final build-input revision `59eec99` changes only hosted workflow baseline selection/validation, so the CI #5 scope rule requires a new hosted run and no local battery rerun. Artifact mapping is in `revision-map.md`. |
...
| BLOCK-011 | Exact revision accounting | `CLOSED` | `revision-map.md` maps the complete local battery to `ce3b4b3`/`8da5ee8` and the later hosted-only workflow chain to `b57293c`/`59eec99` with CI #5 evidence at `e01ac1b`. Dependency, clean-room, symbol-graph, and release assembly/signature gates are explicitly retained at the post-`ce3b4b3` battery. |
